 ...
 */ 
var SecondViewPrice2 = function (container, model) {
	 ...
	 * 
	 */
	var pictures = container.find("#picture_area");
	...
	var id_list = model.getAllDishesDatabase();
	var img_list;
	for (i = 1; i < id_list.length; i++){

		img_list = model.getImage(i);

		var img_temp=document.createElement("img");
		img_temp.src= "images/";
    	img_temp.src+= img_list;
    	img_temp.id="picture";
    	var foo = document.getElementById("fooBar");
    	//foo.appendChild(img_temp);
	}
	//pictures.html(id_list);
	pictures.html(model.getNumberOfGuests);

	

	//pictures.html(img_list);
	...
	//description.html(model.getDescription(1));

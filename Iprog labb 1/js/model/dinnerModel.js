...
var DinnerModel = function() {
	...
	this.getSelectedDish = function(type) {
		...
		for(dish in dishes){
			if(dishes[dish].type == type) {
				...
				//	type_string += dishes[dish].name;
				//}
				//else{
					dish_string += " " + dishes[dish].name;
				//}
			}
		}
		return dish_string;
			...
			for(menu_dish in menu){
				...
				for(dish in dishes){
					if(menu[menu_dish] == dishes[dish].id ){
						//menu_string += " Matchat id:t! Rätt: "
						menu_string += dishes[dish].name + " ";
					}
				}
			}
		}
		...
		for(menu_dish in menu){
			for(dish in dishes){
				if(dishes[dish].id ==menu[menu_dish]){
					//ingredient_string+= "Vi har matchat med: " + menu[menu_dish] + " och " + dishes[dish].id;
					//price += dishes[dish].
					var ingredient_array = dishes[dish].ingredients;
					for(ingredient in ingredient_array){
						ingredient_string += ingredient_array[ingredient].name + " ";
					}
				}
			}
		}
		...
		for(dish in dishes){
			if(id == dishes[dish].id){
				...
				temp+= "Typen på id:t är " + search_type;
					//Id:t är okej att lägga in, vi fortsätter
	...
	this.getAllDishesDatabase = function(){
		for(dish in dishes){
	...
	this.firstDescription = function (id){
		for(dish in dishes){
			if(dishes[dish].id == id) {
				return dishes[dish].description;
			}
		}
	}
	...
	this.getImages = function (){
		...
		image_array.pop(2);
		for (dish in dishes){
				image_array.push(dishes[dish].image);
		}
		return image_array;
	}
	...
	//		for(dish in dishes){
	//			if(menu[menu_dish] == dishes[dish].id)
	//				menu_picture += dishes[dish].image + " ";
	//
	//		}
	//
	//	}
...
// Our own function to return a description
...
//			for(dish in dishes){
//				if(menu[menu_dish] == dishes[dish].id)
//					menu_description += dishes[dish].description;
//
//			}
//
//		}
...
this.getName = function (id){
	...
	// Our own function to return the ingredients for a specific dish

	this.getIngredients = function(id){
		for(dish in dishes){
			if(dishes[dish].id == id){
				var temp_array = dishes[dish].ingredients;
				var ingredient_array = [];
				for(ingredient in temp_array){
					//ingredient_array.push(temp_array[ingredient].quantity);
					//ingredient_array.push(temp_array[ingredient].unit);
					//ingredient_array.push(temp_array[ingredient].name);
					ingredient_array += temp_array[ingredient].quantity + " ";
					ingredient_array += temp_array[ingredient].unit + " ";
					ingredient_array+= temp_array[ingredient].name + " ";
				}
